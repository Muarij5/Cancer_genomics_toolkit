# ============================================================
# post_fix_check.R (CN_LOSS_THRESHOLD version)
# Run AFTER Module2_finalized.R (with CN_LOSS_THRESHOLD changed
# to 0.6) is sourced and APP_DATA is loaded.
# ============================================================

COHEN_D_THRESHOLD <<- 0.35
PARTIAL_COHEN_D_THRESHOLD <<- 0.25
CORRELATION_THRESHOLD <<- 0.25
# CN_LOSS_THRESHOLD should already be 0.6 in the sourced file --
# not reset here since it's not a threshold this script tunes directly.
...
# ------------------------------------------------------------
# PART 2 -- full FR re-score on the stratified subset
# ------------------------------------------------------------
...
cat(sprintf("\nAFTER CN_LOSS_THRESHOLD=0.6 -- TP=%d FP=%d FN=%d\n", tp, fp, fn))
cat(sprintf("Precision=%.3f  Recall=%.3f\n", tp/(tp+fp), tp/(tp+fn)))
cat("\n(Compare to BEFORE this change: TP=4 FP=9 FN=12, Precision=0.308, Recall=0.250)\n")