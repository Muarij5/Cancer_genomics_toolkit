...
DOSE_UNIT     <- "nM"                        # mathews_screening_data doses are in nM
RESPONSE_TYPE <- "viability"                 # mathews_screening_data is %viability
...
# ------------------------------------------------------------
data("mathews_screening_data", package = "synergyfinder")
sf_raw <- mathews_screening_data
...
ref_bliss_obj <- CalculateSynergy(reshaped, method = "Bliss", correct_baseline = "non")
ref_loewe_obj <- CalculateSynergy(reshaped, method = "Loewe", correct_baseline = "non")
...
write.csv(merged[, out_cols], "benchmark_comparison_results.csv", row.names = FALSE)
...
for (pk in pair_keys) {
  ...
  sub_df$inhibition <- 1 - (sub_df$Response / 100)
...
write.csv(raw_diag_df, "D:/Tool_genomic/module_1/raw_single_agent_points.csv", row.names = FALSE)
...
if (length(ref_curve_compare) > 0) {
  ...
  write.csv(curve_compare_df, "D:/Tool_genomic/module_1/curve_fit_comparison2.csv", row.names = FALSE)
  cat("Single-agent curve comparison (yours vs. synergyfinder\'s own fit) written to: curve_fit_comparison.csv\n")
